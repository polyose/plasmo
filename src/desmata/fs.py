...
from desmata.interface import Cell as CellBase
from desmata.models import Cell
from desmata.protocols import CellEnv, DBFactory, Loggers, UserspaceFiles
...
@dataclass
class CellFiles(CreatePathAttrs, CellEnv):
    home: Path
...
class DesmataFiles(CreatePathAttrs, UserspaceFiles):
    ...
    def __post_init__(self):
        db_file = self.state.absolute()
        self.log.msg(f"creating sqlite engine at {db_file}")
        self.engine = self.db_source.get_engine(self)
    ...
    def home_for(self, *, cell_type: type[CellBase]) -> Path:
        ...
        self.log.msg.debug(f"DB engine: {self.engine}")
